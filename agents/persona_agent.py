...
from agents.chat_latency import trace_attach_to_payload, trace_span
...
class PersonaAgent(BaseAgent):
    ...
    def _process_message(self, msg: Message, role: str = "observer"):
        ...
        if _cycle_depth not in ("fast", "normal"):
            _cycle_depth = "normal"
        ...
        _fast_cycle = _cycle_depth == "fast" and role == "conversant"
        self._process_msg_fast_cycle = _fast_cycle
        _chat_llm_primary = str(
            os.environ.get("HAROMA_CHAT_LLM_PRIMARY", "1") or "1",
        ).strip().lower() in ("1", "true", "yes", "on")
        _fast_llm_raw = str(os.environ.get("HAROMA_FAST_LLM", "") or "").strip().lower()
        if _fast_llm_raw in ("1", "true", "yes", "on"):
            _fast_llm_allowed = True
        elif _fast_llm_raw in ("0", "false", "no", "off"):
            _fast_llm_allowed = False
        else:
            # Unset: match primary packed-LLM chat (CPU users have no GPU auto-enable).
            _fast_llm_allowed = _chat_llm_primary
        ...
        _sync_ctx = nullcontext() if _fast_cycle else s.neural_sync()
        with _sync_ctx:
            ...
            if _ground and getattr(s, "meaning_lexicon", None) and not _fast_cycle:
            ...
            if role == "conversant":
                ...
                if _fast_cycle:
                    _conv_skip = _conv_skip | s.process_gate._CHAT_FAST_EXTRA_SKIP
            ...
            if nlu_result and not _fast_cycle:
                _conv_history = [
                    t.to_dict()
            ...
            if _fast_cycle:
                if _teaching:
                    recall_limit = max(6, min(recall_limit, 12))
                elif _chat_llm_primary and role == "conversant" and has_external:
                    recall_limit = min(max(recall_limit, 14), 28)
                else:
                    recall_limit = min(recall_limit, 2)
            elif is_internal:
            ...
            recalled = s.memory.merge_recall_with_prime(
                recalled, recall_limit, fast_cycle=_fast_cycle
            )
            ...
            if not _fast_cycle and getattr(s, "http_chat_inflight", 0) == 0:
                try:
            ...
            _llm_centric = (
                ...
                and (not _fast_cycle or _fast_llm_allowed)
            )
            _memory_forest_seed = ""
            ...
            if has_external and content:
                ...
                if not _fast_cycle and nlu_result and _discourse_result is not None:
            ...
            if not _over_budget():
                ...
                if getattr(s, "http_chat_inflight", 0) == 0 and not _fast_cycle:
                    try:
            ...
            if _gate_decisions.get("temporal_bind", True):
                if _fast_cycle:
                    temporal_pos = s.temporal.get_temporal_position(cycle_id, None)
                else:
            ...
            if not _over_budget() and not _fast_cycle:
        ...
        _packed_llm = _is_real_module(getattr(s, "llm_backend", None))
        _user_message = bool(str(_utt).strip())
        _conversant_chat = (
            role == "conversant" and has_external and _user_message and not is_internal
        )
        _llm_primary_path = (
            _packed_llm
            and _chat_llm_primary
            and _conversant_chat
            and (not _fast_cycle or _fast_llm_allowed)
        )
        _llm_classic_path = (
            _packed_llm
        ...
        # Fast HTTP: cap packed LLM wait via ``HAROMA_FAST_LLM_TIMEOUT_SEC`` (explicit),
        # else ``HAROMA_FAST_LLM_DEFAULT_TIMEOUT_SEC`` (default 120s). Set either to ``0``
        # to fall back to global ``HAROMA_LLM_CONTEXT_TIMEOUT_SEC`` (often 600s).
        _fast_llm_t_raw = str(os.environ.get("HAROMA_FAST_LLM_TIMEOUT_SEC", "") or "").strip()
        if _fast_cycle and _over_budget():
            _llm_primary_path = False

        _kg_for_llm = None
        if _llm_centric or _llm_primary_path or _llm_classic_path:
            try:
        ...
        _sync_ctx_post = nullcontext() if _fast_cycle else s.neural_sync()
        with _sync_ctx_post:
            ...
            _utter_style = "conversational" if (_fast_cycle or role == "conversant") else None
            ...
            elif _multi_goal and not _fast_cycle and active_goals:
    ...
    def _run_idle_cycle(self):
        ...
        try:
            ...
            print(f"[Persona:{self.agent_id}] http_chat_inflight check error: {_hc}", flush=True)
    ...
    def _chat_visible_response(
        ...
        *,
        ...
        Uses deliberation/LLM-produced action text only; no pre-classification
        of user intent (name/math/etc.) into fixed reply lines.
        """
        _ = (user_text, identity)  # Reserved for future post-LLM filters; API stable.
        return resolve_chat_visible_text(action, llm_context)
        ...
        *,
        ...
        out: Dict[str, Any] = {
            "response": self._chat_visible_response(
                action,
                user_text=user_text,
                identity=getattr(episode, "identity", None) or {},
                llm_context=getattr(episode, "llm_context", None) or {},
            ),
            "cycle": cid,
